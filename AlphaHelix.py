...
if __name__=="__main__":
    ...
    lines = [None,]*6
    scat = [None,]*6
    arrow_vector=[0, -0.8*radius_min]
    intersecting_dotted_line_coords = ary([[-radius_max, 0], [radius_max, 0]])
    for sex in range(6):
        ...
        rotated_outline_tmp = rotate_list_of_points(output_outline, sex*angle_between_sextant)
        c = 'C'+str(sex)
        [lines[sex], ]= ax.plot(ary(rotated_outline_tmp).T[0], ary(rotated_outline_tmp).T[1], color=c, linewidth=0.5)
        ...
        scat[sex] = ax.scatter(x_orig, y_orig, facecolor='white', edgecolor=c, linewidths=1.0, s=3.0**2)
    [intersecting_dotted_line, ]= ax.plot(*intersecting_dotted_line_coords.T, linestyle='dashed', color='dimgrey')
    #The dotted line represents the direction in the cross-section view which is parallel to (i.e. in the) the xy plane.
    #make the scatter plot
    with writer.saving(fig, "EvenlyDistributed.mp4", num_frames):
        for frame in range(num_frames):
            theta = get_theta(frame)
            plt.suptitle( 'z={0:.2f} mm, viewing plane is tilted by'.format(round(get_z_of_central_column(theta),2)) )
            ...
            for sex in range(6):
                ...
                rotated_transformed_list = rotate_list_of_points(transformed_list, sex*angle_between_sextant)
                scat[sex].set_offsets( rotated_transformed_list )
                # subsequent bodge to rotate the WHOLE circle
                scat[sex].set_offsets( rotate_list_of_points(scat[sex].get_offsets(), theta) )
                rotated_outline = rotate_list_of_points(ary(lines[sex].get_data()).T, step_size)
                lines[sex].set_data(ary(rotated_outline).T[0], ary(rotated_outline).T[1])
            rotated_intersection_line = rotate_list_of_points(ary(intersecting_dotted_line.get_data()).T, step_size)
            intersecting_dotted_line.set_data(ary(rotated_intersection_line).T[0], ary(rotated_intersection_line).T[1])
            arrow = ax.annotate(s=r'$\theta$', xy=[0,0], xytext=rotate(arrow_vector, theta), arrowprops={'arrowstyle': '<-'})
            # arrow = ax.annotate(s=r'$\vec{z}$', xy=[0,0], xytext=rotate(arrow_vector, theta), arrowprops={'arrowstyle': '<-'})
            writer.grab_frame()
            arrow.remove()
        fig.clf()
